...
public class DepartmentService implements DepartmentManager{
	 ...
	 * @throws Exception
	 */
	public List<Department> listAllUser(String parentId) throws Exception {
		List<Department> departmentList = this.listSubDepartmentByParentId(parentId);
		for(Department depar : departmentList){
			depar.setTreeurl("user/listUsers.do?DEPARTMENT_ID="+depar.getDEPARTMENT_ID());
			depar.setSubDepartment(this.listAllUser(depar.getDEPARTMENT_ID()));
			depar.setTarget("treeFrame");
			depar.setIcon("static/images/user.gif");
		}
		return departmentList;
	...
	public List<Department> listAllDept() throws Exception{
		return (List<Department>) dao.findForList("DepartmentMapper.edit", new Department());
	}
}
